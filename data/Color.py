import pygame.color

BLACK = pygame.color.Color(0, 0, 0)
WHITE = pygame.color.Color(255, 255, 255)
RED = pygame.color.Color(255, 0, 0)
RED_HOVER = pygame.color.Color(110, 0, 0)
GREY = pygame.color.Color(170, 170, 170)
GREY_LIGHTEN = pygame.color.Color(200, 200, 200)
DARK_GREY = pygame.color.Color(37, 37, 37)
GREEN = pygame.color.Color(24, 240, 10)
GREEN_HOVER = pygame.color.Color(24, 110, 10)
PURPLE = pygame.color.Color(102, 53, 168)
DARK_PURPLE = pygame.color.Color(7, 0, 114)
YELLOW_ORANGE = pygame.color.Color(223, 172, 14)
BACKGROUND_SHADOW = pygame.color.Color(0, 0, 0, 100)

RARITY_COLORS = {
    "common": pygame.color.Color(187, 186, 186),
    "uncommon": pygame.color.Color(74, 176, 16),
    "rare": pygame.color.Color(12, 107, 220),
    "epic": pygame.color.Color(109, 25, 167),
    "legendary": pygame.color.Color(225, 100, 13)
}



FOND_INTERACTIONS = pygame.color.Color(248, 196, 113)

